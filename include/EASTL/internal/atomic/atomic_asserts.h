...
#define EASTL_ATOMIC_STATIC_ASSERT_TYPE(type)							\
	...
	static_assert(eastl::is_nothrow_default_constructible<type>::value, "eastl::atomic<T> : Template Typename T must be nothrow default constructible!");
...
namespace internal
{
	...
	{
		/**
		 * class Test { int i; int j; int k; }; sizeof(Test2) == 96 bits
		 *
