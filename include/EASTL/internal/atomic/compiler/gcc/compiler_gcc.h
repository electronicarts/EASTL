 ...
 * into locks. We would rather have user-code explicity use locking primitives if their code cannot
 ...
 * Firstly, it depends on the -march setting on clang whether or not it calls out to libatomic.
 ...
 *
 * Therefore, the follow static_assert is commented out for the time being.
 *
...
/**
 * NOTE:
 * Why we do the cast to the unsigned fixed width types for every operation even though gcc/clang builtins are generics?
 ...
 * struct UserType { uint8_t a,b; }; This given struct is 16 bytes in size but has only 8 byte alignment.
 ...
 * Therefore you'll notice we always cast each atomic ptr type to the equivalent unsigned width type when doing the atomic operations.
 */
