...
//
	...
	#define EASTL_ARCH_ATOMIC_X64_XOR_FETCH_PRE_COMPUTE_DESIRED(ret, observed, val) \
		ret = ((observed) ^ (val))

	#define EASTL_ARCH_ATOMIC_X64_XOR_FETCH_POST_COMPUTE_RET(ret, prevObserved, val) \
		ret = ((prevObserved) ^ (val))
	...
	#define EASTL_ARCH_ATOMIC_XOR_FETCH_RELAXED_128(type, ret, ptr, val)	\
		EASTL_ARCH_ATOMIC_X64_OP_128_IMPL(type, ret, ptr, val, RELAXED,		\
										  EASTL_ARCH_ATOMIC_X64_XOR_FETCH_PRE_COMPUTE_DESIRED, \
										  EASTL_ARCH_ATOMIC_X64_XOR_FETCH_POST_COMPUTE_RET)
